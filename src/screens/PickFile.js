...
const PickFile = ({ navigation }) => {
    ...
    const handleDocumentSelection = React.useCallback(async () => {
        try {
            ...
            setFileResponse((prev) => {
                const newArray = _.concat(prev, response);
                const filterArray = _.uniqBy(newArray, 'name');
                console.log('filterArray: ', filterArray);
                const filterType = _.filter(filterArray, (file) => {
                    if (file.type === 'application/pdf' || _.startsWith(file.type, 'image/')) {
                        return true;
                    }
                    return false;
                });
    ...
    return (
        <View style={styles.container}>
            ...
            {
                (path && _.startsWith(path.type, 'image/')) &&
                <View
